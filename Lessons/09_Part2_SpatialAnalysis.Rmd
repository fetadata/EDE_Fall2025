...
huc8sub_sf <- st_read(here('./Data/Spatial/NCHUC8.shp')) %>% 
...
mapview(huc8sub_sf, highlightOptions(color='blue',stroke=T, fill = T, fillColor='yellow'))
mapview(huc8_sf, highlightOptions(fill = T, fillColor='grey'))
...
HUC_12_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', 
  colClasses = c('HUC_12' = 'factor')) %>% 
  mutate(Join8 = substr(HUC_12, 0, 8)) %>% 
  group_by(Join8) %>% 
...
WindHUC8 <- huc8_sf %>% 
  left_join(HUC_12_raw,
            by = c('HUC_8' = 'Join8'))

#View the outputs
ggplot()+
  geom_sf(data=WindHUC8, aes(fill=SumWind))
...
huc8select <- huc8_sf_utm %>% 
  st_filter(triCo, .predicate = st_intersects)

mapview(huc8select) + mapview(triCo)
#Intersect the HUC_8s
HUC8triCo <- huc8select %>% st_intersection(triCo)
...
#Right order
ggplot() +
  geom_sf(data = state_sf_utm, color='red',size=6) +
  geom_sf(data = counties_sf_utm, aes(fill = ALAND), color = 'white')  +
