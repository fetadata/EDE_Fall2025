...
When the data is served in GeoJSON format, we can ingest it directly in to R. Follow these steps: - Navigate to <https://gis-fws.opendata.arcgis.com/> - Scroll down to the Explore Categories area. Select Biota (for example) - Search for "Sea Turtle Nesting" - Select the first return ("2023 Sea Turtle Nesting Survey TAFB results") and open its [link](https://duke.is/SeaTurtleNestingData) - Locate the APIs dropdown and copy the link to the GeoJSON option.

If you have difficulty, the link you want is: - <https://services.arcgis.com/QVENGdaPbd4LUkLV/arcgis/rest/services/survey123_3d7e213f47a94f1b829aa7cf54eab095_results/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson>

This a link to a spatial dataset in GeoJSON format. Let's read this dataset in and explore it: - Create a spatial dataframe from the link, filtering for only records in North Carolina (STATE == "NC") - Reveal its structure - What coordinate reference system (crs) does it use?
...
Now, see if you can find, import, and display data from Durham's Open Data portal - Do a web search for Durham Open Data - Browse and choose a Feature Service (e.g. Landmarks & Points of Interest) - Find the GeoJSON endpoint and import the dataset - Explore properties of the dataset - Make a map - Export your map to an image or HTML document
...
If we dissect the URL pointing to the feature server, we can discover other data hosted on the server. Both the gas plant and the landslide data sources begin with \<"<https://services.nconemap.gov/secure/rest/services/>"\> -- the address of the server hosting these layers. - Open <https://services.nconemap.gov/secure/rest/services/> in your browser - Note all the resources hosted there - **MapServer** and **FeatureServer** items host vector data - Select the **NC1Map_Health** Feature Server - View the metadata for this feature service - This feature service hosts four layers... - Select the Hospitals (0) Layer link - We see more metadata, including a list of all the fields in this spatial dataset - At the bottom, we see "Supported Operations" including a Query link - Select the Query link at the bottom of the page
