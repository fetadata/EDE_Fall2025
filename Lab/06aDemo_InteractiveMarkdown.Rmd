...
plot_it_now <- function(the_parameter,start_year,end_year){
  #Construct the plot
  nutrient_data %>% 
    filter(year4 > start_year & year4 < end_year) %>% 
    ggplot(aes(
...
selectInput(
  ...
  label = 'select the nutrient for display',
  choices = c('tn_ug','no23','po4'),
  selected = 'no23'
)
...
sliderInput(
  ...
  value = c(mean(nutrient_data$year4),max(nutrient_data$year4)),
  step = .5,
  round = T,
  sep = ''
)
...
renderText(
  paste("only losers select ",input$the_nutrient,", LOSER!!!!!")
)
