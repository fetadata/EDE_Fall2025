...
```{r}
#Exercise 1:
Density <- ggplot(EPAair)+
  geom_density(aes(Ozone,fill=as.factor(Year)), alpha = 0.5, position="stack")+
...
lm.o.pm25 <- (lm(formula = PM2.5 ~ Ozone, EPAair))
summary(lm.o.pm25)
# p value = <2e-16; R-squre = 0.1314
...
lm.o.pm25.plot <- ggplot(lm.o.pm25, aes(x=Ozone, y=PM2.5))+
...
ozonebyall.reg <- lm(data=EPAair.subset,PM2.5~Ozone+Year+Month+mean_Lng+mean_Lat)
step(ozonebyall.reg)
...
bestreg <- lm(formula = PM2.5 ~ Ozone + Month + mean_Lng + mean_Lat, data = EPAair.subset)
summary(bestreg)
